...
TypescriptDataSchemaGenerator.generate = async (config) => {
    const schema = await _a.buildSchema(config);
    return (0, generate_ts_schema_1.generateTypescriptDataSchema)(schema, {
        ...
        identifier: _a.createIdentifier(config),
    });
};
...
TypescriptDataSchemaGenerator.buildSchema = async (config) => {
    const adapter = _a.getAdapter(config);
    await adapter.initialize();
    const schema = new schema_representation_1.Schema(new schema_representation_1.Engine(_a.getDBEngineType(config)));
...
TypescriptDataSchemaGenerator.createIdentifier = (config) => {
    ...
    const identifier = `ID${_a.removeNonAlphaNumericChars(identifierHash)}`;
    return identifier;
};
