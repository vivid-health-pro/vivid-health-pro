...
const utils_1 = require("../../../../../../foundation/utils");
 ...
 * @internal
 */
const loadOrCreateMultipartUpload = async ({ ctx, s3Config, data, size, contentType, bucket, accessLevel, keyPrefix, key, contentDisposition, contentEncoding, metadata, abortSignal, checksumAlgorithm, optionsHash, resumableUploadsCache, expectedBucketOwner, }) => {
    const finalKey = keyPrefix !== undefined ? keyPrefix + key : key;
    let cachedUpload;
    if (!resumableUploadsCache) {
        utils_2.logger.debug('uploaded cache instance cannot be determined, skipping cache.');
    ...
    if (cachedUpload) {
        ...
        const finalCrc32 = checksumAlgorithm === constants_1.CHECKSUM_ALGORITHM_CRC32
            ? await (0, utils_1.calculateContentCRC32)(ctx, data)
            : undefined;
