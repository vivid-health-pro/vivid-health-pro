 ...
 */
function filterScriptingPatterns(str) {
    if (!str)
        return '';
    // Check for dangerous JavaScript patterns
    if (constants_1.scriptingPatterns.some((pattern) => pattern.test(str))) {
        return '';
    }
    return str.trim();
}
exports.filterScriptingPatterns = filterScriptingPatterns;
/**
 ...
 */
function escapePropertyValue(propertyValue) {
    // First check if it's a reserved keyword
    if (keywords_1.default.has(propertyValue)) {
...
exports.escapePropertyValue = escapePropertyValue;
/**
...
function buildBindingExpression(prop) {
    ...
    const identifier = typescript_1.factory.createIdentifier(escapedPropertyValue);
    if (prop.bindingProperties.field === undefined) {
        return identifier;
    }
    const escapedFieldValue = escapePropertyValue(prop.bindingProperties.field);
    const propertyAccess = typescript_1.factory.createPropertyAccessChain(identifier, typescript_1.factory.createToken(typescript_1.SyntaxKind.QuestionDotToken), escapedFieldValue);
...
function buildBindingWithDefaultExpression(prop, defaultValue) {
    ...
    const leftExpr = prop.bindingProperties.field === undefined
        ? typescript_1.factory.createIdentifier(escapedProperty)
        : typescript_1.factory.createPropertyAccessChain(typescript_1.factory.createIdentifier(escapedProperty), typescript_1.factory.createToken(typescript_1.SyntaxKind.QuestionDotToken), escapePropertyValue(prop.bindingProperties.field));
...
function buildCollectionBindingExpression(prop) {
    return prop.collectionBindingProperties.field === undefined
        ...
        : typescript_1.factory.createPropertyAccessExpression(typescript_1.factory.createIdentifier('item'), escapePropertyValue(prop.collectionBindingProperties.field));
}
